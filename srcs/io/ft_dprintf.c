...
static const char	*print_text(int fd, t_print *tab, const char *format)
{
	char	*next;
	int		size;
...
static int	read_input(int fd, t_print *tab, const char *format)
{
	while (*format)
	{
		...
		if (*format == '%')
			...
			format = print_text(fd, tab, format);
		if (!format)
...
int	ft_dprintf(int fd, const char *format, ...)
{
	...
	printed = read_input(fd, tab, format);
	if (printed == -1)
		return (-1);
	va_end(tab->args);
	printed = tab->len;
